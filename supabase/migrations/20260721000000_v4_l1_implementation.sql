-- MJRH V4 — Layer 1: THE ABSOLUTE CORE (v3.7 - ATOMIC & DEFENSIVE)
...
-- [TABLE] Structural Mutation Facts (ActorID added)
...
BEGIN
    -- 1. Identity Guard (Fixed: Atomic Lock and Lifecycle Check)
    -- Lock the identity record to prevent race conditions during binding
    SELECT is_sovereign_root, sovereign_owner_id, lifecycle_status 
    INTO _identity_rec 
    FROM identities WHERE id = _new.identity_id FOR UPDATE;

    IF _identity_rec.lifecycle_status = 'archived' THEN
        RAISE EXCEPTION 'IDENTITY_ARCHIVED_VIOLATION: Cannot create/move nodes for an archived identity.' USING ERRCODE = 'P1110';
    END IF;

    -- 2. Sovereign Root Immutability
    ...
    -- 3. Sovereign Root Identity Check
    ...
    -- 4. Zombie Guard (No mutations under ARCHIVED ancestors)
    ...
    -- 5. Sovereign Identity Binding (Atomic)
...
BEGIN
    ...
    VALUES (NEW.id, TG_OP, CASE WHEN TG_OP = 'UPDATE' THEN OLD.node_path ELSE NULL END, NEW.node_path, auth.uid());
...
SET search_path = v4_l1, public AS $$
BEGIN RETURN (SELECT ltree2text_array(node_path) FROM nodes WHERE id = _node_id); END;
$$ LANGUAGE plpgsql STABLE;
