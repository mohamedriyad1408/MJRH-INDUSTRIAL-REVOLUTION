-- MJRH V4 — Layer 1: HARDENED CORE (v3.6 - PRODUCTION READY)
...
DECLARE
    _p_path ltree;
BEGIN
    -- Fix #1: Only increment on UPDATE
...
-- Fix #2: Facts in AFTER trigger
CREATE OR REPLACE FUNCTION v4_l1.fn_l1_emit_fact() RETURNS trigger AS $$
BEGIN
...
CREATE TRIGGER trg_l1_after_update AFTER UPDATE OF node_path ON v4_l1.nodes FOR EACH ROW EXECUTE FUNCTION v4_l1.fn_propagate_path();
CREATE TRIGGER trg_l1_after_fact AFTER INSERT OR UPDATE ON v4_l1.nodes FOR EACH ROW EXECUTE FUNCTION v4_l1.fn_l1_emit_fact();
