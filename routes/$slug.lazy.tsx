...
function TenantEntryPage() {
  ...
  if (!tenant) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-slate-50" dir={dir}>
        <Card className="max-w-md w-full"><CardContent className="p-8 text-center text-muted-foreground">{t("errors.pageNotFoundDetail")}</CardContent></Card>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#ccfbf1,#f8fafc_45%,#e0f2fe)] p-4 flex items-center justify-center" dir={dir}>
      <Card className="w-full max-w-lg overflow-hidden border-0 shadow-2xl">
        <div className="p-8 text-center text-white" style={{ background: `linear-gradient(135deg, ${color}, #0f172a)` }}>
          ...
          <p className="text-sm text-white/80 mt-2">{t("landing.portalSubtitle", "مدخل العملاء والموظفين والمالك")}</p>
        ...
        <CardContent className="p-6 space-y-3">
          <Button asChild className="w-full h-14 text-base font-black bg-teal-600 hover:bg-teal-700">
            <Link to="/customer-portal" search={{ tenant: tenant.slug } as any}>
              <Users className="w-5 h-5 ms-2" /> {t("landing.customerLogin", "دخول العملاء")}
            </Link>
          </Button>

          <Button asChild variant="secondary" className="w-full h-14 text-base font-black">
            <Link to="/join/$slug" params={{ slug: tenant.slug }}>
              <UserPlus className="w-5 h-5 ms-2" /> {t("landing.registerCustomer", "تسجيل عميل جديد")}
            </Link>
          </Button>

          <Button asChild variant="outline" className="w-full h-14 text-base font-black">
            <Link to="/login" search={{ tenant: tenant.slug } as any}>
              <LogIn className="w-5 h-5 ms-2" /> {t("landing.staffLogin", "دخول الموظفين والمالك")}
            </Link>
          </Button>

          {tenant.business_phone && <a href={`tel:${tenant.business_phone}`} className="block text-center text-sm text-muted-foreground hover:underline pt-2 font-mono">{t("landing.contactBiz", "اتصل بالمغسلة:")} {tenant.business_phone}</a>}
