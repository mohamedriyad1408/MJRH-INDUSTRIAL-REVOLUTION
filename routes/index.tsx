...
import { Loader2, Building2, LogIn, Layers, Hotel, Hospital, Utensils, Factory, Globe, ArrowLeft, Check } from "lucide-react";
    ...
    <div className="min-h-screen bg-white text-slate-900" dir={dir}>
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-slate-100">
        <div className="mx-auto max-w-6xl px-6 h-[64px] flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button asChild size="sm" className="rounded-full bg-slate-900 hover:bg-black"><Link to="/login">دخول</Link></Button>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-6 py-16">
        ...
        <section className="max-w-3xl mx-auto text-center space-y-6 py-8">
          <Badge variant="outline" className="rounded-full border-slate-200">منصة تشغيل للمؤسسات من 1 إلى 500 فرع</Badge>
          ...
          <p className="text-lg text-slate-600 leading-8 max-w-2xl mx-auto">
            مصممة للمؤسسات الكبرى. فندق، مستشفى، سلسلة مطاعم 50 فرع — نفس النواة، تقارير مجمعة، تخصيص كامل لكل فرع بدون إعادة بناء.
          </p>
          <div className="flex justify-center gap-3">
            <Button asChild size="lg" className="rounded-full h-11 px-7 bg-slate-900 hover:bg-black"><Link to="/landing">استكشاف المنصة <ArrowLeft className="w-4 h-4 ms-2" /></Link></Button>
            <Button asChild size="lg" variant="outline" className="rounded-full h-11"><Link to="/login">دخول</Link></Button>
          </div>
        </section>
        ...
        <section className="py-8">
          <Card className="border shadow-xl rounded-[1.5rem] overflow-hidden max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-3 divide-x border-t bg-white text-center">
              ...
              <div className="p-4"><div className="text-xl font-black">Multi</div><div className="text-xs text-slate-500">شركات</div></div>
              <div className="p-4"><div className="text-xl font-black">∞</div><div className="text-xs text-slate-500">تخصيص</div></div>
            </div>
          </Card>
        </section>
        ...
        <section className="py-10 border-y border-slate-100 my-10">
          <div className="flex flex-wrap justify-center gap-8 text-sm font-medium text-slate-500">
            <span className="flex items-center gap-2"><Hotel className="w-4 h-4" /> فنادق 7 نجوم</span>
            ...
            <span className="flex items-center gap-2"><Factory className="w-4 h-4" /> مصانع</span>
            <span className="flex items-center gap-2"><Globe className="w-4 h-4" /> شركات متعددة الدول</span>
          </div>
        </section>
        ...
        <section id="active-projects-list" className="py-8">
          <div className="flex items-end justify-between mb-6">
            <h2 className="text-2xl font-black">المشاريع النشطة</h2>
            <span className="text-xs text-slate-500">{tenants.length} مشروع</span>
          </div>
          {loading ? (
            <div className="flex justify-center p-10"><Loader2 className="w-6 h-6 animate-spin" /></div>
          ) : errored || !tenants.length ? (
            <Card><CardContent className="p-8 text-center text-sm text-slate-500">لا توجد مشاريع نشطة حالياً</CardContent></Card>
          ) : (
        ...
        <section className="py-16 text-center">
          <h2 className="text-3xl font-black">جاهز للانطلاق؟</h2>
          <p className="text-slate-600 mt-2">نفس المنصة تشغّل مغسلة صغيرة وفندق 7 نجوم</p>
          <div className="mt-6 flex justify-center gap-3">
            <Button asChild className="rounded-full bg-slate-900 h-11 px-7"><Link to="/landing">استكشاف</Link></Button>
            <Button asChild variant="outline" className="rounded-full h-11"><Link to="/login">دخول</Link></Button>
          </div>
        </section>
      </main>

      {/* Footer - Clean, phone only here under copyrights */}
        ...
        <div className="mx-auto max-w-6xl px-6 py-10 flex flex-col md:flex-row items-center justify-between gap-4 text-sm">
          ...
          <div className="flex flex-col items-center md:items-end gap-1 text-xs text-slate-600">
            ...
            <div className="flex gap-4">
              ...
              <Link to="/landing" className="hover:underline">المنصة</Link>
            </div>
          </div>
        </div>
      </footer>
