...
import { ShieldCheck, FileText, Upload, Plus, Trash2, Loader2, ExternalLink, Building2, Clock, CheckCircle2, AlertCircle, DollarSign, Calendar } from "lucide-react";
import { fmtMoney, fmtDate } from "@/lib/format";
import { toast } from "sonner";

export const Route = createFileRoute("/_admin/admin/legal")({
  head: () => ({ meta: [{ title: "الشؤون القانونية والامتثال والعقود - MJRH" }] }),
...
const CONTRACT_TYPES: Record<string, string> = {
  ...
  employment: "عقد عمل وتوظيف قيادي",
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-black text-white tracking-tight flex items-center gap-2">
              <span>الشؤون القانونية والامتثال والعقود (Legal & Compliance HQ)</span>
              <Badge variant="outline" className="border-teal-500 text-teal-400 text-xs font-mono">Verified DB</Badge>
            </h1>
            <p className="text-sm text-slate-400 mt-1">
              إدارة العقود التأسيسية، تراخيص الملكية الفكرية، واتفاقيات الخدمة مع المستأجرين والشركاء مع التوثيق الميداني والرفع الحقيقي للملفات.
            </p>
          </div>
        </div>
          ...
          <DialogContent className="max-w-xl bg-slate-900 border-slate-700 text-slate-100 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 py-2 text-xs md:text-sm">
              <div className="space-y-1.5">
                ...
                <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="مثال: عقد ترخيص استخدام منظومة MJRH (SaaS MSA)" className="bg-slate-950 border-slate-700 text-white" required />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input value={counterparty} onChange={(e) => setCounterparty(e.target.value)} placeholder="مثال: مغسلة دراي تك (Dry Tech)" className="bg-slate-950 border-slate-700 text-white" required />
                </div>
              </div>
