...
export const Route = createFileRoute("/$tenant/crm")({
    ...
    {loading ? <div className="p-12 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto text-teal-600" /></div> : <Tabs defaultValue="loyalty" className="space-y-4">
      ...
      <TabsContent value="loyalty">
        <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><Star className="w-4 h-4 text-amber-500" />{t("crm.loyalty.title", "أفضل العملاء")}</CardTitle></CardHeader><CardContent className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
          {loyalty.map((l) => <div key={l.id} className="rounded-2xl border p-3"><div className="flex justify-between gap-2"><div className="font-black">{l.customers?.full_name ?? t("station.common.customer")}</div><Badge>{tierAr(l.tier, t)}</Badge></div><div className="text-xs text-muted-foreground mt-1">{l.customers?.phone}</div><div className="grid grid-cols-2 gap-2 mt-3 text-sm"><div className="rounded-xl bg-teal-50 p-2"><div className="text-xs text-teal-700">{t("crm.loyalty.points", "النقاط")}</div><b>{l.points}</b></div><div className="rounded-xl bg-slate-50 p-2"><div className="text-xs text-slate-500">{t("crm.loyalty.spend", "إجمالي الشراء")}</div><b>{fmtMoney(l.lifetime_spend, curr)}</b></div></div></div>)}
