...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (hasRole("courier") && !isManager) { nav({ to: "/$tenant/driver" as any }); return; }
    if (hasRole("employee") && !isManager && user) {
      supabase.from("employees").select("id,station,job_role,profile_id,email").or(`profile_id.eq.${user.id},email.eq.${user.email}`).maybeSingle().then(async ({ data }: any) => {
        ...
        if (data?.job_role === "driver") nav({ to: "/$tenant/driver" as any });
  ...
  return (
    <div className="space-y-6" dir={dir}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Link to={"/$tenant/live-map" as any} className="flex items-center gap-2 text-sm text-teal-600 font-bold border border-teal-200 bg-teal-50 px-3 py-2 rounded-lg hover:bg-teal-100 transition">
