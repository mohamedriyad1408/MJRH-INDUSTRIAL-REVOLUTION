...
export default function ReportsPage() {
  ...
  async function load() {
    ...
    if (revenueDelta !== null) insights.push({
      ...
      title: revenueDelta >= 0 ? t("reports.revenueGrowth", "الإيراد طالع عن الشهر السابق") : t("reports.revenueDecline", "الإيراد نازل عن الشهر السابق"),
      body: `${Math.abs(revenueDelta).toFixed(1)}% ${revenueDelta >= 0 ? t("common.increase", "زيادة") : t("common.decrease", "انخفاض")} مقارنة بالشهر السابق.`,
      action: revenueDelta >= 0 ? t("reports.growthAction", "كرر مصدر النمو: راجع أكثر الخدمات والموظفين إنتاجية.") : t("reports.declineAction", "راجع أسباب الانخفاض: عدد الطلبات، متوسط الفاتورة، والعملاء غير النشطين."),
    });
    if (Number(bottleneck[1]) > Math.max(8, units.length * 0.35)) insights.push({ tone: "warn", title: t("reports.bottleneckVisible", "عنق زجاجة ظاهر"), body: `${bottleneck[1]} قطعة متكدسة في مرحلة ${t("stage." + bottleneck[0], bottleneck[0])}.`, action: t("reports.bottleneckAction", "انقل موظف مؤقتًا لهذه المحطة أو افتح وردية قصيرة لتفريغ التكدس.") });
    if (qcRate > 8 || recleanCount > 0) insights.push({ tone: "bad", title: t("reports.qualityLeak", "تسريب جودة"), body: `${qcFailed} فشل QC و ${recleanCount} مرتجع تنظيف.`, action: t("reports.qualityLeakAction", "افتح محطة QC يوميًا قبل التغليف وراجع الفني/الخدمة المتكررة في الفشل.") });
    if (unpaidValue > totalRevenue * 0.25) insights.push({ tone: "warn", title: t("reports.highReceivables", "تحصيل مؤجل عالي"), body: `${t("common.receivables", "الآجل")} الحالي ${fmtMoney(unpaidValue)} من إجمالي ${fmtMoney(totalRevenue)}.`, action: t("reports.highReceivablesAction", "فعّل رسالة واتساب آلية قبل التسليم وامنع خروج الطلب غير المدفوع إلا بإذن.") });
    if (lowStock.length) insights.push({ tone: "warn", title: t("reports.stockWarning", "مخزون معرض للنفاد"), body: `${lowStock.length} صنف وصل أو تعدّى حد إعادة الطلب.`, action: t("reports.stockWarningAction", "راجع صفحة المخزون واطلب الكيماويات/الأكياس قبل ضغط التشغيل.") });
    if (!insights.length) insights.push({ tone: "good", title: t("reports.stableOps", "التشغيل مستقر"), body: t("reports.stableOpsBody", "لا توجد مؤشرات خطر واضحة في الفترة المختارة."), action: t("reports.stableOpsAction", "استمر في متابعة الجودة والمخزون يوميًا.") });
  ...
  function exportCSV() {
    if (!data) return;
    const rows = [
      [t("common.branch"), branchId === "all" ? t("common.allBranches") : branches.find((b) => b.id === branchId)?.name ?? "فرع محدد"],
      [t("common.indicator"), t("common.value")],
      [t("finance.revenueTab"), data.totalRevenue],
      [t("accounting.kpi.paidExpenses"), data.totalExpenses],
      [t("accounting.kpi.payableExpenses"), data.payableExpenses],
      [t("reports.payableMonth"), data.payrollAccrual],
      [t("finance.netProfit"), data.netProfit],
      [t("station.common.orders"), data.totalOrders],
      [t("reports.avgInvoice"), data.avgOrder],
      [t("nav./receivables"), data.unpaidValue],
      [t("station.qc.failed"), data.qcFailed],
      [t("station.common.reclean"), data.recleanCount],
      [t("reports.bottleneck"), t("stage." + data.bottleneck[0], data.bottleneck[0])],
    ];
    ...
    toast.success(t("reports.exported", "تم تصدير التقرير"));
  }
      ...
      {loading ? <div className="flex justify-center p-12"><Loader2 className="w-6 h-6 animate-spin text-teal-600" /></div> : data && (
        <div className="space-y-6" dir={dir}>
              ...
              <CardHeader className="pb-3 border-b border-slate-100">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <CardTitle className="text-base font-black flex items-center gap-2 text-slate-900">
                    ...
                    <span>{t("today.surgeMonitor.title", "مرصد كثافة المواعيد وأوقات الذروة الموزعة (Surge Load Monitor)")}</span>
                  </CardTitle>
                  <div className="flex flex-wrap gap-1.5">
                    <Badge variant="outline" className="bg-emerald-50 text-emerald-800 font-bold">🟢 {t("today.surgeMonitor.normal", "عادي")}: {surgeData.normalCount}</Badge>
                    <Badge variant="outline" className="bg-amber-50 text-amber-800 font-bold">🟡 {t("today.surgeMonitor.normalPeak", "ذروة عادية")}: {surgeData.normalPeakCount}</Badge>
                    <Badge variant="outline" className="bg-orange-50 text-orange-800 font-bold">🟠 {t("today.surgeMonitor.mediumPeak", "ذروة متوسطة")}: {surgeData.mediumPeakCount}</Badge>
                    <Badge variant="outline" className="bg-red-50 text-red-800 font-bold">{t("today.surgeMonitor.severePeak", "ذروة شديدة")}: {surgeData.severePeakCount}</Badge>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  {t("today.surgeMonitor.description", "متابعة الضغط التشغيلي على فترات استلام وتسليم المندوب الموزعة على مدار اليوم وغداً (فترات ساعتين متصلتين).")}
                </p>
              </CardHeader>
                ...
                {surgeData.slots.map((st: any) => (
                  <div key={st.slot} className={`p-3 rounded-xl border text-xs space-y-1.5 transition ${st.disabled ? "bg-red-50/80 border-red-300 opacity-90" : `${st.bgClass} bg-white`}`}>
                    ...
                    <div className={`text-[11px] font-bold ${st.colorClass}`}>
                      {st.count} {t("today.surgeMonitor.scheduledOrders", "طلبات مجدولة")}
                    </div>
          ...
          <Card className="border-teal-200 bg-gradient-to-br from-teal-50 to-white">
            <CardHeader><CardTitle className="text-base flex items-center gap-2"><Brain className="w-5 h-5 text-teal-700" />{t("reports.systemTalk")}</CardTitle></CardHeader>
            <CardContent className="grid md:grid-cols-2 gap-3">
              {data.insights.map((x: Insight, i: number) => <InsightCard key={i} item={x} t={t} />)}
            </CardContent>
          </Card>
          ...
          <div className="grid lg:grid-cols-3 gap-4">
              ...
              <CardContent className="space-y-3">
                {Object.entries(data.stageCounts).sort((a: any, b: any) => b[1] - a[1]).map(([stage, count]: any) => {
                  ...
                  return <div key={stage}>
                    <div className="flex justify-between text-xs mb-1"><span>{t("stage." + stage, stage)}</span><span>{count} {t("common.piece", "قطعة")}</span></div>
                ...
                {!Object.keys(data.stageCounts).length && <div className="text-center text-muted-foreground p-6">{t("reports.noPieces")}</div>}
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-3 text-sm">
                <Leak label={t("station.qc.failed")} value={data.qcFailed} danger={data.qcFailed > 0} />
                <Leak label={t("station.common.reclean")} value={data.recleanCount} danger={data.recleanCount > 0} />
                <Leak label={t("reports.labelIssues", "مشاكل مارك/ليبل")} value={data.labelIssues ?? 0} danger={(data.labelIssues ?? 0) > 0} />
                <Leak label={t("reports.lowStock")} value={data.lowStock.length} danger={data.lowStock.length > 0} />
                <Leak label={t("reports.urgentOrders", "طلبات مستعجلة")} value={data.urgent} danger={false} />
              </CardContent>
            </Card>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <Panel title={t("reports.stationsPerformance")} icon={<Clock className="w-4 h-4 text-blue-500" />}>
              {data.stations.map((s: any) => <Row key={s.station} left={t("stage." + s.station, s.station)} mid={`${s.count} ${t("reports.task", "مهمة")}`} right={s.avgHours ? `${s.avgHours.toFixed(1)}${t("common.hoursShort", "س")}` : "—"} />)}
              {!data.stations.length && <Empty t={t} />}
            </Panel>
            <Panel title={t("reports.topEmployees")} icon={<Award className="w-4 h-4 text-amber-500" />}>
              {data.topEmployees.map((e: any, i: number) => <Row key={i} left={`${i + 1}. ${e.name}`} mid={`${e.count} ${t("reports.task", "مهمة")}`} right={`${e.completed} ${t("reports.completed", "منتهي")}`} />)}
              {!data.topEmployees.length && <Empty t={t} />}
            </Panel>
            <Panel title={t("reports.topServices")} icon={<TrendingUp className="w-4 h-4 text-teal-500" />}>
              {data.topServices.map((s: any) => <Row key={s.name} left={s.name} mid={`${s.qty} ${t("common.piece", "قطعة")}`} right={fmtMoney(s.revenue, t("common.egp"))} />)}
              {!data.topServices.length && <Empty t={t} />}
            </Panel>
          </div>
        </div>
      )}
...
function InsightCard({ item, t }: { item: Insight; t: any }) {
  ...
  return <div className={`rounded-2xl border p-4 ${cls}`}><div className="flex items-center gap-2 font-black"><AlertTriangle className="w-4 h-4" />{item.title}</div><p className="text-sm mt-2 text-slate-700">{item.body}</p><div className="mt-3 text-xs font-bold text-slate-900">{t("reports.suggestedAction", "الإجراء المقترح:")} {item.action}</div></div>;
}
...
function Empty({ t }: { t: any }) { return <div className="text-xs text-muted-foreground text-center py-4">{t("common.noData", "لا توجد بيانات")}</div>; }
