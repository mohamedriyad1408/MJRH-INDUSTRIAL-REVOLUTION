...
function DryingAssemblyStation() {
 ...
 <CardHeader className="bg-muted/40 pb-3"><div className="flex flex-wrap items-center justify-between gap-2"><CardTitle className="text-base flex items-center gap-2"><PackageCheck className="w-4 h-4 text-teal-600" /> {t("order.orderNo", "طلب #{order}").replace("{order}", String(g.order.order_number))}<Badge variant="outline">{g.units.length} {t("station.common.pieces")}</Badge>{g.units.some((u) => u.label_status !== "labeled") && <Badge variant="destructive">{t("station.common.markIssue")}</Badge>}</CardTitle><div className="flex items-center gap-2"><Button asChild size="sm" variant="outline"><Link to={"/$tenant/orders/$id" as any} params={{ id: g.orderId } as any}>{t("station.common.openOrder")} <ArrowLeft className="w-3 h-3 me-1" /></Link></Button><SorterReturnDialog orderId={g.orderId} orderNumber={g.order?.order_number || "?"} tenantId={null} onDone={load} /></div></div><div className="text-xs text-muted-foreground">{g.order.customer_name ?? "—"} · {g.order.customer_phone ?? ""}</div>
<OmnipresentOrderBanner order={g.order as any} customer={(g.order as any)?.customers} className="mt-2" /></CardHeader>
