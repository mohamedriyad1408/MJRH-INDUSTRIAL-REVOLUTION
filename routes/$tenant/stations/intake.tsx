...
import { Loader2, Inbox, Plus, RefreshCw, Truck, ArrowRight } from "lucide-react";
      ...
      {loading ? (
        ...
        <Card className="rounded-3xl shadow-sm border">
          ...
          <CardContent className="p-4 space-y-3">
            {pickups.length === 0 ? <p className="text-center text-xs text-slate-400 py-8 font-bold">لا توجد شحنات استلام معلقة</p> : pickups.map((p) => (
              <div key={p.id} className="p-3.5 rounded-2xl border bg-white flex flex-wrap items-center justify-between gap-3 shadow-2xs">
                <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button asChild size="sm" variant="outline" className="rounded-xl font-bold border-amber-300 text-amber-900 hover:bg-amber-50"><Link to={"/$tenant/orders/new" as any}><ArrowRight className="w-4 h-4 ms-1" /> فتح في فاتورة الاستلام</Link></Button>
                </div>
              </div>
            ))}
          </CardContent>
