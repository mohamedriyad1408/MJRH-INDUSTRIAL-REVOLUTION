...
import { ArrowLeft, CheckCircle2, Image as ImageIcon, Loader2, Package, PackageCheck, ShieldCheck, Shirt, Tags, Sparkles, Truck } from "lucide-react";
...
function PackingStation() {
 ...
 setBusy(null);
  ...
  async function fastTrackPackAndReady(g: Group) {
    const c = checks(g);
    if (!c.okToReady && !c.okToPack) return toast.error("لا يمكن التغليف والتجهيز قبل حل المرتجعات أو المارك");
    setBusy(g.orderId);
    try {
      const ids = g.units.filter((u) => !["ready"].includes(u.current_stage)).map((u) => u.id);
      if (ids.length > 0) {
        await supabase.from("service_units").update({ current_stage: "ready", staff_notes: "تمت مراجعة التغليف وتجهيز التسليم" }).in("id", ids);
      }

      let assignedDriverNote = "";
      if (g.order.status !== "delivered") {
        const { data: ordInfo } = await supabase.from("orders").select("order_type,assigned_driver_employee_id,tenant_id,branch_id").eq("id", g.orderId).single();
        if (ordInfo?.order_type === "delivery" && !ordInfo.assigned_driver_employee_id) {
          const { data: couriers } = await supabase.from("employees").select("id,full_name").eq("tenant_id", ordInfo.tenant_id).eq("is_active", true).or("station.eq.delivery,job_role.eq.courier").limit(1);
          if (couriers && couriers.length > 0) {
            await supabase.from("orders").update({ assigned_driver_employee_id: couriers[0].id, pickup_status: "assigned" }).eq("id", g.orderId);
            assignedDriverNote = ` | تم التوجيه الجغرافي آلياً للمندوب: ${couriers[0].full_name}`;
          }
        }
      }

      await supabase.from("orders").update({ status: "ready" }).eq("id", g.orderId).neq("status", "cancelled");

      const actorName = activeActor?.full_name || "التغليف النهائي";
      await supabase.from("order_status_history").insert({
        order_id: g.orderId,
        from_status: g.order.status,
        to_status: "ready",
        changed_by: user?.id,
        notes: `الأتمتة التشغيلية الشاملة (1-Click Pack & Ready): تم إتمام تغليف وتجهيز ${g.units.length} قطعة للتسليم الفوري — نفذه: ${actorName}${assignedDriverNote}`
      });

      toast.success(`تم التغليف الشامل ونقل الطلب لحالة جاهز للتسليم وتوجيه المندوب بنجاح`);
      load();
    } catch (e: any) {
      toast.error("خطأ في التغليف والتجهيز السريع: " + (e?.message || ""));
    } finally {
      setBusy(null);
    }
  }
...
<OmnipresentOrderBanner order={g.order} customer={g.order?.customers} className="mt-2" /></CardHeader>
 ...
 <div className="flex flex-wrap justify-end gap-2"><Button variant="outline" disabled={busy === g.orderId || !c.okToPack} onClick={() => packAll(g)}><CheckCircle2 className="w-4 h-4 ms-1" />{t("station.packing.packAll")}</Button><Button disabled={busy === g.orderId || !c.okToReady} className="bg-emerald-600 hover:bg-emerald-500" onClick={() => markReady(g)}><ShieldCheck className="w-4 h-4 ms-1" />{t("station.packing.markReady")}</Button><Button disabled={busy === g.orderId || (!c.okToReady && !c.okToPack)} className="bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white font-black shadow-md text-xs" onClick={() => fastTrackPackAndReady(g)}><Sparkles className="w-4 h-4 ms-1 text-teal-200" /><span>إتمام التغليف الشامل وتوجيه المندوب آلياً (1-Click Pack & Ready)</span></Button>{g.order.status === "ironing" && <Button disabled={busy === g.orderId} variant="secondary" onClick={() => startPacking(g)}>{t("station.packing.startPacking")}</Button>}</div>
 </CardContent>
 </Card>; })}
 </div>}
 </div>;
