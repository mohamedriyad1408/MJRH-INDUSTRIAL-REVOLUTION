...
function CsPickupNotebook({ tenantId, activeActor, onSuccess }: { tenantId: string; activeActor: ActiveActor | null; onSuccess: () => void }) {
 ...
 return (<Card className="rounded-3xl border-2 border-indigo-500/30 shadow-md bg-gradient-to-br from-indigo-50/50 via-white to-teal-50/30 overflow-hidden">
 ...
 {existingCustomer ? (<Badge className="bg-emerald-600 text-white font-bold text-[10px] mt-1.5 w-full justify-center">🟢 عميل قديم مسجل بالمغسلة</Badge>) : phone.length >= 10 ? (<Badge className="bg-blue-600 text-white font-bold text-[10px] mt-1.5 w-full justify-center">🆕 عميل جديد (سيتم إنشاؤه آلياً)</Badge>) : null}
 </div>
 <div>
...
function CsStationPage() {
 ...
 {loading ? (<div className="flex justify-center p-12"><Loader2 className="w-6 h-6 animate-spin text-teal-600" /></div>) : (<div className="grid md:grid-cols-2 gap-5">
 ...
 {pickups.length === 0 ? <p className="text-center text-xs text-slate-400 py-6 font-bold">لا توجد طلبات استلام معلقة حالياً</p> : pickups.map((p) => (<div key={p.id} className="p-3 rounded-2xl border bg-white flex items-center justify-between gap-3 shadow-2xs">
 <div>
 ...
 <CardContent className="p-4 space-y-3">
 {orders.length === 0 ? <p className="text-center text-xs text-slate-400 py-6 font-bold">لا توجد طلبات معلقة</p> : orders.map((o) => (<div key={o.id} className="p-3 rounded-2xl border bg-white flex items-center justify-between gap-3 shadow-2xs">
 <div>
