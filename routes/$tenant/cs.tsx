...
export const Route = createFileRoute("/$tenant/cs")({
  ...
  return (
    <div className="space-y-6" dir={dir}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button asChild><Link to={"/$tenant/orders/new" as any}><PlusCircle className="w-4 h-4 ms-1" /> {t("orders.newOrder")}</Link></Button>
      </div>
...
function StatCard({ label, value, icon: Icon, tone }: { label: string; value: number; icon: React.ComponentType<{ className?: string }>; tone?: string }) {
  return (
    <Link to={"/$tenant/orders" as any} className="block">
