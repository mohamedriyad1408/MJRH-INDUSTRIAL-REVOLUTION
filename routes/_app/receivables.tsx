...
function ReceivablesPage() {
  ...
  return <div className="space-y-5" dir={dir}>
    ...
    <div className="relative max-w-md"><Search className="w-4 h-4 absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><Input className="pr-9" placeholder={t("receivables.searchPlaceholder", "بحث باسم العميل أو الهاتف")} value={q} onChange={(e) => setQ(e.target.value)} /></div>
    {loading ? <div className="p-12 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto text-teal-600" /></div> : <div className="grid lg:grid-cols-[1fr_420px] gap-4">
      ...
      <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><Wallet className="w-4 h-4" />{t("receivables.ledgerTitle", "آخر قيود العملاء")}</CardTitle></CardHeader><CardContent className="space-y-2">{ledger.map((l) => <div key={l.id} className="rounded-xl border p-3 text-sm"><div className="flex justify-between gap-2"><b>{l.customers?.full_name ?? t("stations.common.customer")}</b><Badge variant={l.direction === "customer_owes" ? "destructive" : "secondary"}>{l.entry_type === "invoice" ? t("receivables.invoice", "فاتورة") : t("receivables.payment", "سداد")}</Badge></div><div className="text-xs text-muted-foreground mt-1">{t("receivables.orderNo", "طلب #")}{l.orders?.order_number ?? "—"} · {fmtDate(l.entry_at)}</div><div className="font-black mt-2">{fmtMoney(l.amount, curr)}</div></div>)}</CardContent></Card>
    </div>}
  </div>;
}
