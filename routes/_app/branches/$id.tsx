...
function BranchDashboardPage() {
  ...
  return <div className="space-y-5" dir={dir}>
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="flex gap-2"><Button variant="outline" onClick={load}>{t("common.refresh")}</Button><Button asChild><Link to="/branches">{t("branchView.btnAllBranches", "كل الفروع")}</Link></Button></div>
    </div>
    ...
    {loading ? <div className="p-12 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto text-teal-600" /></div> : <div className="space-y-5">
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card><CardHeader><CardTitle className="text-base">{t("branchView.ordersTitle", "طلبات اليوم في الفرع")}</CardTitle></CardHeader><CardContent className="space-y-2">{orders.map((o) => <Link key={o.id} to={`/orders/${o.id}` as any}><div className="rounded-xl border p-3 text-sm flex justify-between gap-2"><div><b>{t("orders.orderNo", "طلب #{order}").replace("{order}", String(o.order_number))}</b><div className="text-xs text-muted-foreground">{o.customers?.full_name ?? t("branchView.orderCustomer", "عميل")} · {o.status}</div></div><Badge>{fmtMoney(o.total, curr)}</Badge></div></Link>)}{!orders.length && <Empty text={t("branchView.ordersEmpty", "لا توجد طلبات اليوم")} />}</CardContent></Card>
