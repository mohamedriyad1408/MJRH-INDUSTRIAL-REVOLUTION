...
function DryingAssemblyStation() {
  ...
  if (!canUse) return <Card><CardContent className="p-10 text-center text-muted-foreground">{t("stations.assembly.accessDenied")}</CardContent></Card>;
  ...
  return <div className="space-y-5" dir={dir}>
    <div className="rounded-3xl bg-gradient-to-br from-cyan-700 via-slate-900 to-violet-800 text-white p-5 shadow-xl overflow-hidden relative">
      ...
      <div className="relative flex flex-wrap items-center justify-between gap-3">
        <div><h1 className="text-2xl font-black flex items-center gap-2"><Wind className="w-7 h-7 text-cyan-200" /> {t("stations.assembly.title")}</h1><p className="text-sm text-white/75 mt-1">{t("stations.assembly.subtitle")}</p></div>
        <Button variant="secondary" onClick={load}>{t("common.refresh")}</Button>
      </div>
      <div className="grid grid-cols-5 gap-2 mt-4 text-center">
        <Mini label={t("stations.common.orders")} value={stats.orders} /> <Mini label={t("stations.common.pieces")} value={stats.pieces} /> <Mini label={t("stations.assembly.active")} value={stats.active} /> <Mini label={t("stations.assembly.ready")} value={stats.ready} /> <Mini label={t("stations.common.markIssue")} value={stats.missing} warn />
      </div>
    </div>
    ...
    {loading ? <div className="p-12 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto text-teal-600" /></div> : <div className="space-y-3">
      {!grouped.length && <Card><CardContent className="p-10 text-center text-muted-foreground">{t("stations.assembly.noPieces")}</CardContent></Card>}
      {grouped.map((g) => <Card key={g.orderId} className="overflow-hidden bg-white/85 backdrop-blur">
        <CardHeader className="bg-muted/40 pb-3"><div className="flex flex-wrap items-center justify-between gap-2"><CardTitle className="text-base flex items-center gap-2"><PackageCheck className="w-4 h-4 text-teal-600" /> {t("orders.orderNo", "طلب #{order}").replace("{order}", String(g.order.order_number))}<Badge variant="outline">{g.units.length} {t("stations.common.pieces")}</Badge>{g.units.some((u) => u.label_status !== "labeled") && <Badge variant="destructive">{t("stations.common.markIssue")}</Badge>}</CardTitle><Button asChild size="sm" variant="outline"><Link to="/orders/$id" params={{ id: g.orderId }}>{t("stations.common.openOrder")} <ArrowLeft className="w-3 h-3 me-1" /></Link></Button></div><div className="text-xs text-muted-foreground">{g.order.customer_name ?? "—"} · {g.order.customer_phone ?? ""}</div></CardHeader>
        <CardContent className="p-3 grid md:grid-cols-2 gap-3">
          {g.units.map((u) => <div key={u.id} className={`rounded-2xl border p-3 space-y-3 ${u.label_status !== "labeled" ? "bg-amber-50 border-amber-200" : "bg-white"}`}>
            <div className="grid grid-cols-[58px_1fr] gap-3 items-center"><div className="w-14 h-14 rounded-xl bg-muted border overflow-hidden flex items-center justify-center">{u.photo_url ? <img src={u.photo_url} className="w-full h-full object-cover" /> : <ImageIcon className="w-6 h-6 text-muted-foreground" />}</div><div><div className="flex flex-wrap gap-2 items-center"><span className="font-black">{u.label_code || t("stations.common.noCode")}</span><Badge variant="outline">{u.name}</Badge>{u.service_type === "both" && <Badge className="bg-violet-600"><Shirt className="w-3 h-3 ms-1" /> {t("stations.assembly.afterAssemblyIroning")}</Badge>}</div><div className="text-xs text-muted-foreground mt-1">{t("stations.common.stage")}: {stageAr(u.current_stage, t)} · {t("stations.common.mark")}: {labelAr(u.label_status, t)}</div>{u.reclean_reason && <div className="text-xs text-amber-700 mt-1">{t("stations.common.reclean")}: {u.reclean_reason}</div>}</div></div>
            <Textarea rows={2} placeholder={t("stations.assembly.notePlaceholder")} value={notes[u.id] ?? u.assembly_notes ?? ""} onChange={(e) => setNotes((m) => ({ ...m, [u.id]: e.target.value }))} />
            <div className="flex flex-wrap gap-2 justify-end">
              <Button size="sm" variant="outline" onClick={() => startAssembly(u)} disabled={busy === u.id}><Search className="w-3 h-3 ms-1" /> {t("stations.assembly.startReview")}</Button>
              <label className="inline-flex"><input type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => captureAndSearch(u, e.target.files?.[0] ?? null)} /><Button size="sm" type="button" variant="outline" className="border-cyan-300 text-cyan-700" asChild><span><Camera className="w-3 h-3 ms-1" /> {t("stations.assembly.photoSearch")}</span></Button></label>
              <Button size="sm" variant="outline" className="border-amber-300 text-amber-700" onClick={() => reportLabel(u, "unclear_label")} disabled={busy === u.id}><Tags className="w-3 h-3 ms-1" /> {t("stations.assembly.unclearMark")}</Button>
              <Button size="sm" variant="destructive" onClick={() => reportLabel(u, "missing_label")} disabled={busy === u.id}>{t("stations.assembly.noMark")}</Button>
              <Button size="sm" className="bg-emerald-600 hover:bg-emerald-500" onClick={() => completeAssembly(u)} disabled={busy === u.id || u.label_status === "missing_label"}>{busy === u.id ? <Loader2 className="w-3 h-3 animate-spin ms-1" /> : <CheckCircle2 className="w-3 h-3 ms-1" />} {t("stations.assembly.readyForIroning")}</Button>
            </div>
            {matches[u.id]?.length ? <div className="rounded-2xl border bg-cyan-50 p-2"><div className="text-xs font-black text-cyan-900 mb-2">{t("stations.assembly.searchResults")}</div><div className="grid grid-cols-3 gap-2">{matches[u.id].map((m: any) => <div key={m.id} className="rounded-xl bg-white border p-1 text-center"><img src={m.photo_url} className="h-16 w-full object-cover rounded-lg bg-muted" /><div className="text-[10px] font-bold mt-1 truncate">{m.label_code}</div><div className="text-[10px] text-muted-foreground truncate">{m.name}</div><div className="text-[10px] text-cyan-700 truncate">{t("orders.orderNo", "طلب #{order}").replace("{order}", String(m.orders?.order_number ?? "?"))}</div></div>)}</div></div> : null}
          </div>)}
        </CardContent>
      </Card>)}
    </div>}
