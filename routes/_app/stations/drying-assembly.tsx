      ...
      {grouped.map((g) => <Card key={g.orderId} className="overflow-hidden bg-white/85 backdrop-blur">
        ...
        <CardContent className="p-3 grid md:grid-cols-2 gap-3">
          {g.units.map((u) => <div key={u.id} className={`rounded-2xl border p-3 space-y-3 ${u.label_status !== "labeled" ? "bg-amber-50 border-amber-200" : "bg-white"}`}>
            <div className="grid grid-cols-[58px_1fr] gap-3 items-center"><div className="w-14 h-14 rounded-xl bg-muted border overflow-hidden flex items-center justify-center">{u.photo_url ? <img src={u.photo_url} className="w-full h-full object-cover" /> : <ImageIcon className="w-6 h-6 text-muted-foreground" />}</div><div><div className="flex flex-wrap gap-2 items-center"><span className="font-black">{u.label_code || t("station.common.noCode")}</span><Badge variant="outline">{u.name}</Badge>{u.service_type === "both" && <Badge className="bg-violet-600"><Shirt className="w-3 h-3 ms-1" /> {t("station.assembly.afterAssemblyIroning")}</Badge>}</div><div className="text-xs text-muted-foreground mt-1">{t("station.common.stage")}: {stageAr(u.current_stage)} · {t("station.common.mark")}: {labelAr(u.label_status)}</div>{u.reclean_reason && <div className="text-xs text-amber-700 mt-1">{t("station.common.reclean")}: {u.reclean_reason}</div>}</div></div>
