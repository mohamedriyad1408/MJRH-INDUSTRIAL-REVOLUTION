  ...
  if (isManager) {
    return (
      <div className="space-y-5" dir={dir}>
        ...
        <StationBoard title={t("stations.cleaning.boardTitle", "تحريك الطلبات من الغسيل إلى التجفيف والتجميع")} station="cleaning" incoming="received" current="cleaning" nextStatus="ironing" nextLabel={t("stations.assembly.title")} />
      </div>
    );
  }
    ...
    <div className="space-y-5" dir={dir}>
      <div className="rounded-3xl bg-gradient-to-br from-blue-700 via-slate-900 to-teal-900 text-white p-5 shadow-xl">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-black flex items-center gap-2"><Sparkles className="w-6 h-6" /> {manager ? t("stations.cleaning.titleManager") : t("stations.cleaning.titleWorker")}</h1>
            <p className="text-sm text-white/70">{t("stations.cleaning.subtitle")}</p>
          </div>
          <Button variant="secondary" onClick={load}>{t("common.refresh")}</Button>
        </div>
        <div className="grid grid-cols-3 gap-3 mt-4">
          <MiniStat label={t("stations.common.orders")} value={groups.length} />
          <MiniStat label={t("stations.cleaning.cleanPieces")} value={units.length} />
          <MiniStat label={t("stations.packing.returns")} value={recleanCount} tone={recleanCount ? "warn" : "ok"} />
        </div>
      </div>

      {loading ? <div className="p-8 text-center text-muted-foreground">{t("common.loading")}...</div> : (
        <div className="space-y-3">
          {!groups.length && <Card><CardContent className="p-10 text-center text-muted-foreground">{t("stations.cleaning.noOrders")}</CardContent></Card>}
          {groups.map((g) => (
            <Card key={g.orderId} className="overflow-hidden">
              <CardHeader className="bg-muted/40 pb-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <CardTitle className="text-base flex items-center gap-2">
                    <Package className="w-4 h-4 text-teal-600" /> {t("orders.orderNo", "طلب #{order}").replace("{order}", String(g.order?.order_number ?? "?"))}
                    <Badge variant="outline">{g.units.length} {t("stations.common.pieces")}</Badge>
                    {g.units.some((u) => u.needs_reclean) && <Badge className="bg-amber-500">{t("stations.common.reclean")}</Badge>}
                  </CardTitle>
                  ...
                  <div key={u.id} className="grid grid-cols-[58px_1fr] md:grid-cols-[58px_1fr_auto] gap-3 items-center rounded-xl border p-2 bg-card">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {u.service_type === "both" && <Badge className="bg-blue-600"><Shirt className="w-3 h-3 ms-1" /> {t("stations.common.cleanIron")}</Badge>}
                        {u.needs_reclean && <Badge className="bg-amber-500"><RotateCcw className="w-3 h-3 ms-1" /> {t("stations.common.recleanCleaning")}</Badge>}
                      </div>
                      {u.reclean_reason && <div className="text-xs text-amber-700 mt-1">{interpolate(t("stations.cleaning.recleanReturnHint"), { reason: u.reclean_reason })}</div>}
                      <div className="text-xs text-muted-foreground mt-1">{t("stations.common.stage")}: {u.current_stage}</div>
                    </div>
                    <div className="col-span-2 md:col-span-1 flex gap-2 justify-end">
                      {u.needs_reclean ? (
                        <Button size="sm" onClick={() => resolveReclean(u)}><CheckCircle2 className="w-3 h-3 ms-1" /> {t("stations.cleaning.cleanedAgain")}</Button>
                      ) : (
                        <Button size="sm" variant="outline" onClick={() => markCleaned(u)}><CheckCircle2 className="w-3 h-3 ms-1" /> {t("stations.cleaning.cleaned")}</Button>
                      )}
                    </div>
                  </div>
                ))}
