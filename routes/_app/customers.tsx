...
import { Loader2, Plus, Search, Pencil, Upload } from "lucide-react";
...
type Customer = { id: string; full_name: string; phone: string; email: string | null; address: string | null; notes: string | null; created_at: string };
...
function CustomersPage() {
  ...
  async function save() {
    ...
    const payload = { full_name: editing.full_name, phone: editing.phone, email: editing.email || null, address: editing.address || null, notes: editing.notes || null };
        ...
        <Card>
          <CardContent className="p-0">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.length === 0 && <tr><td colSpan={5} className="p-8 text-center text-muted-foreground">لا يوجد عملاء</td></tr>}
                {filtered.map((c) => (
