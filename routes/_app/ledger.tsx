...
function LedgerPage() {
  ...
  async function load() {
    ...
    if (!tenantId) {
      setLoading(false);
      setLoadErrors([t("ledger.err.noTenant", t("ledger.err.noTenant"))]);
      return;
    }
    setLoading(true);
    setLoadErrors([]);
  ...
  async function createJournal(description: string, sourceType: string | null, sourceId: string | null, lines: any[]) {
    if (!tenantId) throw new Error(t("ledger.err.noTenant", t("ledger.err.noTenant")));
  ...
  async function repairLedger() {
    if (!tenantId) return toast.error(t("ledger.err.noTenant", t("ledger.err.noTenant")));
    setRepairing(true);
    const errs: string[] = [];
    try {
  ...
  async function postMonthAutomatically() {
    try {
      if (isClosed) return toast.error(t("ledger.err.monthClosed", t("ledger.err.monthClosed")));
      if (!tenantId) return toast.error(t("ledger.err.noTenant", t("ledger.err.noTenant")));
      ...
      load();
    } catch (e: any) { toast.error(e.message ?? t("ledger.err.autoPostFailed", t("ledger.err.autoPostFailed"))); }
  }

  async function addManualJournal() {
    try {
      if (!manual.description || !manual.debit_account || !manual.credit_account || !Number(manual.amount)) return toast.error(t("ledger.err.completeData", t("ledger.err.completeData")));
      ...
      load();
    } catch (e: any) { toast.error(e.message ?? t("ledger.err.createFailed", t("ledger.err.createFailed"))); }
  }

  async function closeMonth() {
    if (!confirm(t("ledger.confirm.close", t("ledger.confirm.close")))) return;
  ...
  if (!canUse) return <Card><CardContent className="p-10 text-center text-muted-foreground">{t("ledger.err.ownerOnly", t("ledger.ownerOnly"))}</CardContent></Card>;
  ...
  return <div className="space-y-5" dir={dir}>
    ...
    <div className="grid md:grid-cols-4 gap-3">
      ...
      <Kpi label={t("ledger.kpi.expense", t("ledger.kpi.expenses"))} value={fmtMoney(pnl.expense, curr)} warn />
    ...
    {isClosed && <Card className="border-amber-200 bg-amber-50"><CardContent className="p-4 font-bold text-amber-800">{t("ledger.alert.closed", t("ledger.alert.closed"))}</CardContent></Card>}
    <Card className="border-blue-200 bg-blue-50"><CardContent className="p-4 text-sm text-blue-900"><b>{t("ledger.alert.warning", t("ledger.alert.warning"))}</b></CardContent></Card>
    ...
    {!loading && loadErrors.length > 0 && <Card className="border-red-200 bg-red-50"><CardContent className="p-4 text-sm text-red-900 space-y-2">
      <div className="font-black">{t("ledger.alert.readErr", t("ledger.alert.readErr"))}</div>
      {loadErrors.map((e, i) => <div key={i} className="rounded-lg bg-white/80 border border-red-100 px-3 py-2 text-xs break-words">{e}</div>)}
      <Button size="sm" onClick={repairLedger} disabled={repairing}>{repairing ? <Loader2 className="w-4 h-4 animate-spin ms-1" /> : <RefreshCw className="w-4 h-4 ms-1" />}{t("ledger.btn.repairNow", t("ledger.btn.repairNow"))}</Button>
    </CardContent></Card>}
    ...
    {!loading && !loadErrors.length && accounts.length === 0 && <Card className="border-amber-200 bg-amber-50"><CardContent className="p-4 text-sm text-amber-900 space-y-2">
      <div className="font-black">{t("ledger.alert.noChart", t("ledger.alert.noChart"))}</div>
      <div>{t("ledger.alert.chartHint", t("ledger.alert.chartHint"))}</div>
      <Button size="sm" onClick={repairLedger} disabled={repairing}>{repairing ? <Loader2 className="w-4 h-4 animate-spin ms-1" /> : <RefreshCw className="w-4 h-4 ms-1" />}{t("ledger.btn.createChart", t("ledger.btn.createChart"))}</Button>
    </CardContent></Card>}
    ...
    {loading ? <div className="p-12 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto text-teal-600" /></div> : <Tabs defaultValue="journals" className="space-y-4">
      <TabsList><TabsTrigger value="journals">{t("ledger.tab.journals", t("ledger.tab.journals"))}</TabsTrigger><TabsTrigger value="pl">{t("ledger.tab.pl", t("ledger.tab.pl"))}</TabsTrigger><TabsTrigger value="trial">{t("ledger.tab.trial", t("ledger.tab.trial"))}</TabsTrigger><TabsTrigger value="close">{t("ledger.tab.close", t("ledger.tab.close"))}</TabsTrigger></TabsList>
      <TabsContent value="journals" className="grid lg:grid-cols-[380px_1fr] gap-4">
        <div className="space-y-4"><Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><RefreshCw className="w-4 h-4" />{t("ledger.autoPost.title", t("ledger.autoPost.title"))}</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-sm text-muted-foreground">{t("ledger.autoPost.desc", t("ledger.autoPost.desc"))}</p><Button onClick={postMonthAutomatically} disabled={isClosed} className="w-full">{t("ledger.autoPost.btn", t("ledger.autoPost.btn"))}</Button></CardContent></Card>
        <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><Plus className="w-4 h-4" />{t("ledger.manual.title", t("ledger.manual.title"))}</CardTitle></CardHeader><CardContent className="space-y-3"><Field label={t("ledger.manual.descLabel", t("ledger.manual.descLabel"))}><Input value={manual.description} onChange={(e) => setManual({ ...manual, description: e.target.value })} /></Field><div className="grid grid-cols-2 gap-2"><Field label={t("ledger.manual.debitLabel", t("ledger.manual.debitLabel"))}><AccountSelect accounts={accounts} value={manual.debit_account} onChange={(v) => setManual({ ...manual, debit_account: v })} t={t} /></Field><Field label={t("ledger.manual.creditLabel", t("ledger.manual.creditLabel"))}><AccountSelect accounts={accounts} value={manual.credit_account} onChange={(v) => setManual({ ...manual, credit_account: v })} t={t} /></Field></div><Field label={t("ledger.manual.amountLabel", "common.amount")}><Input type="number" value={manual.amount} onChange={(e) => setManual({ ...manual, amount: e.target.value })} /></Field><Textarea placeholder={t("ledger.manual.memoPlaceholder", t("ledger.manual.memoPlaceholder"))} value={manual.memo} onChange={(e) => setManual({ ...manual, memo: e.target.value })} /><Button onClick={addManualJournal} disabled={isClosed} className="w-full">{t("ledger.manual.saveBtn", t("ledger.manual.saveBtn"))}</Button></CardContent></Card></div>
        <Card><CardHeader><CardTitle className="text-base">{t("ledger.journals.title", t("ledger.journals.title"))}</CardTitle></CardHeader><CardContent className="space-y-3">{journals.map((j) => <JournalCard key={j.id} journal={j} curr={curr} t={t} />)}{!journals.length && <Empty text={t("ledger.journals.empty", t("ledger.journals.empty"))} />}</CardContent></Card>
      </TabsContent>
      <TabsContent value="pl"><Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><FileBarChart className="w-4 h-4" />{t("ledger.pl.title", t("ledger.pl.title"))} {month}</CardTitle></CardHeader><CardContent className="space-y-2">{monthlyPl.map((r) => <Row key={r.code} a={`${r.code} — ${r.name}`} b={r.account_type === "revenue" ? t("ledger.pl.rev", t("ledger.pl.rev")) : t("ledger.pl.exp", t("ledger.pl.exp"))} c={fmtMoney(r.amount, curr)} danger={r.account_type === "expense"} />)}{!monthlyPl.length && <Empty text={t("ledger.pl.empty", t("ledger.pl.empty"))} />}<div className="border-t pt-3 mt-3 flex justify-between font-black text-lg"><span>{t("ledger.kpi.netProfit")}</span><span className={pnl.net >= 0 ? "text-emerald-700" : "text-red-700"}>{fmtMoney(pnl.net, curr)}</span></div></CardContent></Card></TabsContent>
      <TabsContent value="trial"><Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><Scale className="w-4 h-4" />{t("ledger.tab.trial", t("ledger.tab.trial"))}</CardTitle></CardHeader><CardContent className="p-0 overflow-x-auto"><table className="w-full text-sm"><thead className="bg-muted/50"><tr><th className="text-start p-3">{t("ledger.trial.account", t("ledger.trial.account"))}</th><th className="text-end p-3">{t("ledger.manual.debitLabel", t("ledger.manual.debitLabel"))}</th><th className="text-end p-3">{t("ledger.manual.creditLabel", t("ledger.manual.creditLabel"))}</th><th className="text-end p-3">{t("ledger.trial.balance", t("ledger.trial.balance"))}</th></tr></thead><tbody>{trial.map((r) => <tr key={r.account_id} className="border-t"><td className="p-3 font-bold">{r.code} — {r.name}</td><td className="p-3 text-end">{fmtMoney(r.total_debit, curr)}</td><td className="p-3 text-end">{fmtMoney(r.total_credit, curr)}</td><td className="p-3 text-end font-black">{fmtMoney(r.balance, curr)}</td></tr>)}<tr className="border-t bg-muted/30 font-black"><td className="p-3">{t("ledger.trial.total", t("ledger.trial.total"))}</td><td className="p-3 text-end">{fmtMoney(trialTotals.debit, curr)}</td><td className="p-3 text-end">{fmtMoney(trialTotals.credit, curr)}</td><td></td></tr></tbody></table></CardContent></Card></TabsContent>
      <TabsContent value="close"><Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><LockKeyhole className="w-4 h-4" />{t("ledger.close.title", t("ledger.close.title"))}</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-sm text-muted-foreground">{t("ledger.close.desc", t("ledger.close.desc"))}</p><Button variant="destructive" onClick={closeMonth} disabled={isClosed}>{t("ledger.close.btn", t("ledger.close.btn"))} {month}</Button><div className="space-y-2 pt-3">{periods.map((p) => <Row key={p.id} a={`${p.period_start} → ${p.period_end}`} b={p.status === "closed" ? t("ledger.close.closed", t("ledger.close.closed")) : t("ledger.close.open", t("ledger.close.open"))} c={p.closed_at ? fmtDate(p.closed_at) : "—"} />)}</div></CardContent></Card></TabsContent>
    </Tabs>}
  </div>;
}

function AccountSelect({ accounts, value, onChange, t }: { accounts: any[]; value: string; onChange: (v: string) => void; t: any }) { return <Select value={value} onValueChange={onChange}><SelectTrigger><SelectValue placeholder={t("ledger.select.account", t("ledger.trial.account"))} /></SelectTrigger><SelectContent>{accounts.map((a) => <SelectItem key={a.id} value={a.id}>{a.code} — {a.name}</SelectItem>)}</SelectContent></Select>; }
function JournalCard({ journal, curr, t }: { journal: any; curr: string; t: any }) { const debit = (journal.journal_lines ?? []).reduce((s: number, l: any) => s + Number(l.debit), 0); return <div className="rounded-2xl border p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div className="font-black">{journal.description}</div><Badge>{fmtMoney(debit, curr)}</Badge></div><div className="text-xs text-muted-foreground mt-1">{journal.entry_date}</div><div className="mt-2 space-y-1">{(journal.journal_lines ?? []).map((l: any) => <div key={l.id} className="grid grid-cols-[1fr_auto_auto] gap-2 text-xs"><span>{l.chart_accounts?.code} — {l.chart_accounts?.name}</span><span className="text-emerald-700">{t("ledger.manual.debitLabel", t("ledger.manual.debitLabel"))} {Number(l.debit) ? fmtMoney(l.debit, curr) : "—"}</span><span className="text-red-700">{t("ledger.manual.creditLabel", t("ledger.manual.creditLabel"))} {Number(l.credit) ? fmtMoney(l.credit, curr) : "—"}</span></div>)}</div></div>; }
