...
function AccountingPage() {
  ...
  async function addCashAccount() {
    ...
    const { error } = await (supabase as any).from("cash_accounts").insert({ name: cashForm.name.trim(), account_type: cashForm.account_type, opening_balance: opening, current_balance: opening });
    if (error) toast.error(error.message); else { toast.success("تم إضافة الحساب"); setCashForm({ name: "", account_type: "cash", opening_balance: "0" }); load(); }
  }
  ...
  return <div className="space-y-5">
    ...
    {loading ? <div className="p-12 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto text-teal-600" /></div> : <Tabs defaultValue="payroll" className="space-y-4">
      ...
      <TabsContent value="cash" className="grid lg:grid-cols-[360px_1fr] gap-4">
        <div className="space-y-4"><Card><CardHeader><CardTitle className="text-base"><Plus className="w-4 h-4 inline ms-1" />إضافة خزنة/حساب</CardTitle></CardHeader><CardContent className="space-y-3"><Field label="الاسم"><Input value={cashForm.name} onChange={(e) => setCashForm({ ...cashForm, name: e.target.value })} /></Field><Field label="النوع"><Select value={cashForm.account_type} onValueChange={(v) => setCashForm({ ...cashForm, account_type: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="cash">خزنة نقدية</SelectItem><SelectItem value="bank">بنك</SelectItem><SelectItem value="wallet">محفظة</SelectItem><SelectItem value="instapay">InstaPay</SelectItem></SelectContent></Select></Field><Field label="رصيد افتتاحي"><Input type="number" value={cashForm.opening_balance} onChange={(e) => setCashForm({ ...cashForm, opening_balance: e.target.value })} /></Field><Button onClick={addCashAccount} className="w-full">إضافة</Button></CardContent></Card>
