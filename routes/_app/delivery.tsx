...
function DeliveryPage() {
  ...
  const loadReadyOrders = async () => {
    ...
    try {
      ...
      toast.error("فشل جلب الطلبات الجاهزة");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="space-y-1">
        <h1 className="text-2xl font-black flex items-center gap-2">
          <Truck className="w-6 h-6 text-teal-600" /> التوصيل
        </h1>
        <p className="text-muted-foreground text-sm font-medium">الطلبات الجاهزة للتسليم للعملاء</p>
      </div>

      {orders.length === 0 ? (
        <Card className="border-dashed p-12 text-center text-muted-foreground">
          <p className="text-lg font-bold">لا توجد طلبات جاهزة للتوصيل</p>
        </Card>
      ) : (
