import { HealthCard, Kpi, safeFixHref, groupForCheck, readinessAr, answerAr, deliveryIssuesAr, healthGroups, type Check } from "@/components/system-health-components";
    ...
    {!loading && <Card className={deliveryBlockedRows.length ? "border-red-200 bg-red-50" : "border-emerald-200 bg-emerald-50"}>
      ...
      <CardContent className="space-y-2 text-sm">
        {deliveryBlockedRows.length === 0 ? <div className="font-bold text-emerald-800">{t("system.deliveryReadinessOk")}</div> : <>
          ...
          <div className="grid md:grid-cols-2 gap-2">{deliveryBlockedRows.map((r) => <Link key={r.order_id} to={`/orders/${r.order_id}` as any}><div className="rounded-xl border bg-white/80 p-3 text-xs hover:shadow-sm"><div className="font-black">{t("orders.orderNo", "طلب #{order}").replace("{order}", String(r.order_number))}</div><div className="text-muted-foreground mt-1">{deliveryIssuesAr(r.issue_codes, t)}</div><div className="flex flex-wrap gap-1 mt-2"><Badge variant="outline">{t("orders.piecesCount")} {r.total_units}</Badge>{r.label_issue_count > 0 && <Badge variant="destructive">مارك {r.label_issue_count}</Badge>}{r.reclean_count > 0 && <Badge className="bg-amber-500">{t("orders.reclean")} {r.reclean_count}</Badge>}{r.not_qc_count > 0 && <Badge variant="outline">QC {r.not_qc_count}</Badge>}{r.unpaid > 0 && <Badge variant="destructive">{t("system.check.unpaid.title")}</Badge>}</div></div></Link>)}</div>
        </>}
      </CardContent>
    </Card>}
